...
from sfm_simu2 import adjust_c2w_matrices
...
class SFM:
    def __init__(self):
        ...
        cam = np.genfromtxt('simu3/camera.csv', delimiter=',', dtype=np.float64)
        self.camera = cam.reshape(-1, 3, 4)
        # self.camera = to_c2w(self.camera)

        self.points_3d = np.genfromtxt('simu3/points.csv', delimiter=',', dtype=np.float64)
        ...
        corr = np.genfromtxt('simu3/corr.csv', delimiter=',')
        corr = corr.transpose()
    ...
    def triangulate(self, m1, m2):
        ...
        np.save('simu3/points_2ds.npy', points_2ds)
        p_2d_1 = points_2ds[:, 0]
        p_2d_2 = points_2ds[:, 1]
    ...
    def add_camera(self, m1, m2):
        ...
        print("match points", len(p3d), len(p2d))
        ...
        self.points_2d = p2d
        self.vis = vis
        np.save('simu3/optimized_camera.npy', optimized_camera)
        np.save('simu3/optimized_points_3d.npy', optimized_points_3d)
        np.save('simu3/vis.npy', vis)
        if m1 == 2 or m1 == 4:
...
def test():
    camera = np.load('simu3/optimized_camera.npy')
    points_3d = np.load('simu3/optimized_points_3d.npy')
    print('center', points_3d.mean(0))
    ...
    create_json(cameras.tolist(), frame_ids, 0.08040715440186917, 'simu3/transforms_train.json')
...
#     cameras = np.load('simu3/optimized_camera.npy')
#     points_3d = np.load('simu3/optimized_points_3d.npy')
#     frame = 6
...
#     frame_ids = [f"frame{i:03}" for i in range(0, 140, 10)]
#     frame_ids = sorted(frame_ids)
#     create_json(c2ws.tolist(), frame_ids, 0.08040715440186917, 'simu3/transformer_train.json')
#

def test_lego():
    c2ws = load_json('simu3/transforms_train_lego.json')
    c2ws[:, :3, 1:3] *= -1
    visualization(np.array(c2ws), np.load('simu3/optimized_points_3d.npy'))
