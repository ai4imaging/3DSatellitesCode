...
import os
os.environ["OPEN3D_RENDERING_ENGINE"] = "OpenGL"
...
def visualization(camera, points_3d, grad_color=True):
    ...
    if random_color:
        ...
        #
        if grad_color:
            frustum_color = generate_color_gradient([91, 155, 233], [250, 60, 68], 14)
        else:
            frustum_color = np.tile(frustum_color_r, (len(c2ws_est), 1))
    ...
    cam_params.extrinsic = np.array([
        [1.54381748e-02, -7.49120309e-01, -6.62254049e-01, 1.05886242e+00],
        [5.30635889e-01, -5.55228835e-01, 6.40426807e-01, 1.62688104e-01],
        [-8.47459271e-01, -3.61302787e-01, 3.88938401e-01, 4.09176305e+01],
        [0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 1.00000000e+00]
    ])
    intrinsic = o3d.camera.PinholeCameraIntrinsic()
    intrinsic.set_intrinsics(1920, 1061, 918.85295342, 918.85295342, 959.5, 530.0)
    ...
    # 保存截图
    vis.capture_screen_image(f"{image_name}.png")
    ...
    # # 可选：图像宽高、焦距、主点位置等
    # print("\n=== Intrinsic Details ===")
    # print(f"Width: {camera_params.intrinsic.width}")
    # print(f"Height: {camera_params.intrinsic.height}")
    # print(f"Fx: {camera_params.intrinsic.intrinsic_matrix[0, 0]}")
    # print(f"Fy: {camera_params.intrinsic.intrinsic_matrix[1, 1]}")
    # print(f"Cx: {camera_params.intrinsic.intrinsic_matrix[0, 2]}")
    # print(f"Cy: {camera_params.intrinsic.intrinsic_matrix[1, 2]}")
    #
    # # 销毁窗口
    # vis.destroy_window()
