...
class SFM:
    def __init__(self):
        self.K = np.array([[4122, 0, 180], [0, 4122, 180], [0, 0, 1]], dtype=np.float64)
        with open('simu4/matches.pkl', 'rb') as file:
            self.matches = pickle.load(file)
        with open('simu4/keypoints.pkl', 'rb') as file:
            self.key_points = pickle.load(file)
        cam = np.genfromtxt('simu4/camera.csv', delimiter=',', dtype=np.float64)
        self.camera = cam.reshape(-1, 3, 4)
        # self.camera = to_c2w(self.camera)

        self.points_3d = np.genfromtxt('simu4/points.csv', delimiter=',', dtype=np.float64)
        ...
        corr = np.genfromtxt('simu4/corr.csv', delimiter=',')
        corr = corr.transpose()
            ...
            for frame in range(point.shape[0]):
                ...
                for j, key in enumerate(self.key_points[frame]):
                    if pt[0] == key[0] and pt[1] == key[1]:
                        ...
                        break
        np.save('simu4/cam.npy', self.camera)
        np.save('simu4/points3d.npy', self.points_3d)
    ...
    def triangulate(self, m1, m2):
        ...
        np.save('simu4/points_2ds.npy', points_2ds)
        p_2d_1 = points_2ds[:, 0]
        p_2d_2 = points_2ds[:, 1]
        ...
        points_3d = np.tile(self.points_3d.mean(0), (len(points_2ds), 1))
    ...
    def add_camera(self, m1, m2):
        ...
        for rela in matches:
            if (m1, rela[0]) not in self.map_2d_3d:
                continue
            if abs(self.key_points[m1][rela[0]][1] - self.key_points[m2][rela[1]][1]) > 10 or abs(
                    self.key_points[m1][rela[0]][0] - self.key_points[m2][rela[1]][0]) > 10:
                continue
        ...
        print('Optimizing:', m1, m2,  'num points',len(p3d_id))
        ...
        self.points_2d = p2d
        self.vis = vis
        np.save('simu4/optimized_camera.npy', optimized_camera)
        np.save('simu4/optimized_points_3d.npy', optimized_points_3d)
        np.save('simu4/points_2d.npy', p2d)
        np.save('simu4/visibility.npy', vis)
        if m1 == 2 or m1 == 5:
...
def test():
    camera = np.load('simu4/optimized_camera.npy')
    points_3d = np.load('simu4/optimized_points_3d.npy')
    ...
    distance = np.linalg.norm(camera_t - point_mean, axis=1)
    print(distance)

    frame_ids = [f"frame{i:03}" for i in range(0, 140, 10)]
    frame_ids = sorted(frame_ids)
    create_json(cameras.tolist(), frame_ids, 0.087267, 'simu4/transforms_train.json')
...
def test_projection():
    K = np.array([[4122, 0, 180], [0, 4122, 180], [0, 0, 1]], dtype=np.float64)
    cameras = np.load('simu4/cam.npy')
    points_3d = np.load('simu4/points3d.npy')
    frame = 2
    ...
    frame_ids = [f"frame{i:03}" for i in range(0, 140, 10)]
    frame_ids = sorted(frame_ids)
    create_json(c2ws.tolist(), frame_ids, 0.08040715440186917, 'simu4/transformer_train.json')


def test_lego():
    c2ws = load_json('simu4/transforms_train_lego.json')
    c2ws[:, :3, 1:3] *= -1
    visualization(np.array(c2ws), np.load('simu4/optimized_points_3d.npy'))
